...
namespace ACManager.Views
{
    internal class BotManagerView : IDisposable
    {
        ...
        #region Gems Variable Declarations
        internal HudList Gems { get; set; }
        internal HudTextBox GemKeyword { get; set; }
        internal HudTextBox GemHeading { get; set; }
        internal HudButton AddGem { get; set; }
        #endregion
        ...
        public BotManagerView(FilterCore parent, CoreManager core)
        {
            try
            {
                ...
                #region Gem Settings
                Gems = View != null ? (HudList)View["GemList"] : new HudList();
                Gems.Click += Gems_Click;
                
                GemKeyword = View != null ? (HudTextBox)View["GemKeyword"] : new HudTextBox();

                GemHeading = View != null ? (HudTextBox)View["GemHeading"] : new HudTextBox();
                GemHeading.Change += GemHeading_Change;

                AddGem = View != null ? (HudButton)View["AddGem"] : new HudButton();
        ...
        private void Gems_Click(object sender, int row, int col)
        {
            try
            {
                if (!row.Equals(0))
                {
                    ...
                    for (int i = 0; i < Filter.Machine.Utility.BotSettings.GemSettings.Count; i++)
                    {
                        if (Filter.Machine.Utility.BotSettings.GemSettings[i].Name.Equals(gemToDelete.Text))
                        {
                            Filter.Machine.Utility.BotSettings.GemSettings.RemoveAt(i);
                            break;
                        }
                    }
        ...
        {
            SetupGemsList();
            foreach (GemSetting gemSetting in Filter.Machine.Utility.BotSettings.GemSettings)
            {
        ...
        private void AddGem_Hit(object sender, EventArgs e)
        {
            try
            {
                if (Core.Actions.CurrentSelection != 0 && Core.WorldFilter[Filter.Machine.Core.Actions.CurrentSelection] != null)
                {
                    WorldObject worldObject = Core.WorldFilter[Filter.Machine.Core.Actions.CurrentSelection];
                    if (worldObject.ObjectClass.Equals(ObjectClass.Gem))
                    {
                        if (!string.IsNullOrEmpty(GemKeyword.Text))
                        {
                            if (!string.IsNullOrEmpty(GemHeading.Text))
                            {
                                ...
                                {
                                    Name = Core.WorldFilter[Filter.Machine.Core.Actions.CurrentSelection].Name,
                                    Keyword = GemKeyword.Text,
                                    Heading = double.Parse(GemHeading.Text)
                                };

                                if (Filter.Machine.Utility.BotSettings.GemSettings.Contains(gemSetting))
                                {
                                    for (int i = 0; i < Filter.Machine.Utility.BotSettings.GemSettings.Count; i++)
                                    {
                                        if (Filter.Machine.Utility.BotSettings.GemSettings[i].Equals(gemSetting))
                                        {
                                            Filter.Machine.Utility.BotSettings.GemSettings[i] = gemSetting;
                                        }
                                    }
                                ...
                                {
                                    Filter.Machine.Utility.BotSettings.GemSettings.Add(gemSetting);

                                    HudList.HudListRowAccessor row = Gems.AddRow();
                                    using (HudStaticText text = new HudStaticText())
                                    {
                                        text.Text = Core.WorldFilter[Filter.Machine.Core.Actions.CurrentSelection].Name;
                                        row[0] = text;
                                    }
